...
//Guarantee its precision for first ten bit.
	double FM_CALL __do_exp(double in) FMTHROW;
	template <typename T, typename std::enable_if<std::is_floating_point<T>::value,
		bool>::type = true>
	FM_FORCE_INLINE T FM_CALL Exp(T ty) FMTHROW {
		return __do_exp(ty);
	}

  	double FM_CALL __do_pow(double in,double a) FMTHROW;
	template <typename T>
	FM_FORCE_INLINE T FM_CALL Pow(T x,T a) FMTHROW {
		return __do_pow(x,a);
	}
} // namespace fm
