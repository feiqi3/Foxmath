...
template<typename T>
T FM_CALL FM_INLINE lerp( T a,T b,FMFLOAT t) FMTHROW{
    return (1 - t) * a + t* b;
}
class vector4;
...
template<typename T>
T FM_CALL FM_INLINE clamp( T a,T min,T max){
...
FM_CALL FMFLOAT FM_INLINE inline degToRad(FMFLOAT a)FMTHROW{
    static FMFLOAT inv180mulPI = FMFLOAT(1)/FMFLOAT(180)*PI;
    return a * PI * inv180mulPI;
}

FM_CALL FMFLOAT FM_INLINE inline radToDeg(FMFLOAT a) FMTHROW{
