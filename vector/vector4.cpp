...
const vector4 FM_INLINE FM_CALL
vector4::operator+(const vector4 &b) const noexcept {
...
const FMFLOAT FM_INLINE FM_CALL vector4::dot(const vector4 &b) const noexcept {
  simd::_fm_vec4 veca = simd::fmLoadVecP(__data._v);
  simd::_fm_vec4 vecb = simd::fmLoadVecP(b.__data._v);
  return simd::fmVecDot(veca, vecb);
}

  const vector4 FM_INLINE FM_CALL vector4::cross(const vector4 &b) const noexcept {
    simd::_fm_vec4 veca = simd::fmLoadVecP(__data._v);
    simd::_fm_vec4 vecb = simd::fmLoadVecP(b.__data._v);
    ...
    FMFLOAT FM_INLINE FM_CALL vector4::maxElement() const noexcept {
    simd::_fm_vec4 vec = simd::fmLoadVecP(__data._v);
    return simd::fmMaxElemOfVec(vec);
  }

    FMFLOAT FM_INLINE FM_CALL vector4::minElement() const noexcept {
    simd::_fm_vec4 vec = simd::fmLoadVecP(__data._v);
    return simd::fmMinElemOfVec(vec);
  }

    FMFLOAT FM_INLINE FM_CALL vector4::length() const noexcept {
    simd::_fm_vec4 vec = simd::fmLoadVecP(this->__data._v);
    FMFLOAT ff = simd::fmVecDot(vec, vec);
    return std::sqrt(ff);
  }

    vector4 FM_INLINE FM_CALL vector4::square() const noexcept {
    simd::_fm_vec4 vec = simd::fmLoadVecP(this->__data._v);
    return vector4(simd::fmVecMul(vec, vec));
  }

    vector4 FM_INLINE FM_CALL vector4::sqrt() const noexcept {
    simd::_fm_vec4 vec = simd::fmLoadVecP(this->__data._v);
    return simd::fmVecSqrt(vec);
  }

  std::ostream &operator<<(std::ostream &out, const vector4 &s) {
    out << "(" << s.__data._v[0] << "," << s.__data._v[1] << ","
        << s.__data._v[2] << "," << s.__data._v[3] << ")";
    return out;
  }
...
bool FM_INLINE FM_CALL hasNan(const vector4& in)noexcept{
    return std::isnan(in[0]) || std::isnan(in[1]) 
    || std::isnan(in[2]) || std::isnan(in[3]);
}
